...
def print_surgery_rules(R, D, labels, g_col, inv_trans=lambda x,y : y):
    ...
    def print_rule(r, i_rule):
        ...
        if(len(I_male)>0):
            if D.iloc[I_male[0]]['v']:
                print(lpad(prefix('MALE',j), conform_str(i_rule, np.hstack(D.iloc[I_male]['source']))))
            else:
                print(lpad(prefix('FEMALE',j), conform_str(i_rule, np.hstack(D.iloc[I_male]['source']))))
            j += 1
            print()
        ...
        def print_range_literal(d, j, i_literal, i_rule):
            ...
            print(lpad(prefix(s,j), conform_str(i_rule, i_literal)))
        ...
        for s in sets:
            ...
            if(len(I_inc)>0):
                print(lpad(prefix(s['pos'], j), conform_str(i_rule, np.hstack(D.iloc[I_inc]['source']))))
                for i in I_inc:
                    print(lpad(lprefix(labels.get(D.iloc[i]['name'], D.iloc[i]['name'])), conform_str(i_rule, np.hstack(D.iloc[i]['source']))))
                    j += 1
                print()
            ...
            if(len(I_exc)>0):
                print(lpad(prefix(s['neg'], j), conform_str(i_rule, np.hstack(D.iloc[I_exc]['source']))))
        ...
        if(len(I_ben)>0):
            print(lpad(prefix('Benzodiazepine == %.1f' % float(D.iloc[I_ben[0]]['v']),j), conform_str(i_rule, np.hstack(D.iloc[I_ben]['source']))))
            j += 1
            print()
    ...
    for r in R:
        ...
        print(lpad(s, '%.1f%% (all), %d/%d, %.1f%%/%.1f%%, neg %d/%d' % (m, n0, n1, m0, m1, mt0, mt1)))
        print('( ')
        print_rule(r, i)
        i+=1
    print(')')
