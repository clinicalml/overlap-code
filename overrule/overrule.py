...
from . import support, ruleset, overlap

from .support import SVMSupportEstimator, SupportEstimator
from .overlap import SupportOverlapEstimator, OverlapEstimator
from .ruleset import RulesetEstimator, BCSRulesetEstimator
...
class OverRule(OverlapEstimator):
    ...
    def __init__(self,
        ...
        if overlap_estimator == 'support':
            ...
            self.O = SupportOverlapEstimator(support_estimator, alpha_s,
        ...
        self._init_ruleset_estimator(ruleset_estimator, alpha_r,
    ...
    def _init_ruleset_estimator(self, ruleset_estimator, alpha, **kwargs):
        ...
        if inspect.isclass(ruleset_estimator) and \
           issubclass(ruleset_estimator, RulesetEstimator):
            self.RS = ruleset_estimator(**kwargs)
        elif isinstance(ruleset_estimator, RulesetEstimator) or \
            isinstance(ruleset_estimator, base.BaseEstimator):
...
class OverRule2Stage(OverlapEstimator):
    ...
    def __init__(self,
        ...
        if isinstance(overlap_rs_estimator, RulesetEstimator):
            ...
            self.RS_o.set_params(gamma=0, n_ref_multiplier=0) #Make sure negative samples not used
        else:
    ...
    def rules(self, as_str=False, transform=None, fmt='%.3f', labels={}):
        ...
        return (M_s.rules(as_str=as_str, transform=transform, fmt=fmt, labels=labels),
